...
class VeAct : BaseFragment(R.layout.activity_ve) {
    ...
    private fun initBrush() {
        brushUtils = BrushUtils(
            requireContext(),
            { bitmap ->
                //save
                logEvent("DrawScrScr_IconOk_Clicked")
                DeviceUtil.arrVe.clear()
                DeviceUtil.arrVe.add(
                    ImageObj(
                        id = "bitmap${System.currentTimeMillis() / 1000}",
                        path = null,
                        bitmap = bitmap,
                        isSelected = false
                    )
            ...
            onBack = {
                logEvent("PencilScr_IconBack_Clicked")
                DeviceUtil.arrVe.clear()
                onBackPress(R.id.veAct)
            },
            onUndo = {
                logEvent("PencilScr_IconUndu_Clicked")
            },
            onRedo = {
                logEvent("PencilScr_IconRedu_Clicked")
            },
            onDone = {
                logEvent("PencilScr_IconDone_Clicked")
            },
        )
        view?.let {
            v->
